...
{
    public class Cheb1Filter : AbstractFilter
    {
        [DllImport("DFilter.dll")]
        private static extern Int32 cheby1(WraperISCMS.CallBackCancel callback, Int32 n, double w1, double w2, double R, double[] Num, double[] Den, double[] ZRe, double[] ZIm, double[] PRe, double[] PIm, ref double ga, Int32 type);
        ...
        public Cheb1Filter(Dictionary<string, AbstractFilter> filters_form) : this()
        {
        ...
        protected bool GetNumDenCoef(WraperISCMS bw)
        {
            if (updated_coef == false)
            {
                ...
                ret = cheby1(new WraperISCMS.CallBackCancel(bw.IsCancelChaeck), ((Int32)Order), 2.0 * F1 / Fs, 2.0 * F2 / Fs, R, NumCoef, DenCoef, pzArray.ZRe, pzArray.ZIm, pzArray.PRe, pzArray.PIm, ref pzArray.ggain, ttype);
