...
{
    public partial class FirLSTab : UserControl
    {
        public FirLSFilter firlsfil;
        NumberFormatInfo provider = new NumberFormatInfo();

        private int validFg;

        public FirLSTab(FilterProperty fs, AbstractFilter fil)
        {
            int nTmp;
            InitializeComponent();
            validFg = 0;
            provider.NumberDecimalSeparator = ".";
            this.firlsfil = (FirLSFilter)fil;
            ...
            textBox5.Text = firlsfil.weights[0].ToString() + " " + firlsfil.weights[1].ToString();

            this.Dock = DockStyle.Fill;

            listitem.Add(new myclas("Symmetric and Odd Length ", "type1", new Bitmap(GetType().Assembly.GetManifestResourceStream("DFDLab.DFDTools.LinearPhaseSpecBMP.LinearPhase1.bmp"))));
            listitem.Add(new myclas("Symmetric and Even Length", "type2", new Bitmap(GetType().Assembly.GetManifestResourceStream("DFDLab.DFDTools.Cheb1SpecBMP.Cheb1High.bmp"))));
        ...
        private bool ValidTb1()
        {
            ...
            validFg = (validFg & ~1) | (Convert.ToInt32(Ret == false));
            return Ret;
        }
        ...
        private bool ValidTb5()
        {
            ...
            if (textBox5.Text.CompareTo("") != 0)
            {
                ...
                if (WT.Count < 250)
                {
                    ...
                    {
                        firlsfil.weights[i] = Convert.ToDouble(WT[i].ToString(),provider);
                    }
                }
                Ret = firlsfil.ValidWeights(WT.Count);
            }
        ...
        {
            firlsfil.TypeFilt = ((myclas)comboBox1.SelectedItem).nName;
            bool flagValidate = ValidTb1();
